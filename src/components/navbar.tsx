...
export default function Navbar() {
  ...
  if (loading) {
    return (
      <nav className={`w-full px-6 py-4 flex items-center justify-between ${isLanding ? "absolute top-0 left-0 z-50 text-white" : "bg-white border-b text-foreground"}`}>
        <Link href="/" className="text-xl font-bold">Minvu Serviu</Link>
  ...
  return (
    <nav
      className={`w-full px-6 py-4 flex items-center justify-between ${
        isLanding
          ? "absolute top-0 left-0 z-50 text-white bg-black/30 backdrop-blur-sm"
      ...
      {mobileOpen && (
        <div
          className={`absolute top-full left-0 w-full flex flex-col items-center gap-4 py-6 md:hidden ${
            isLanding ? "bg-black/80 backdrop-blur-sm text-white" : "bg-white border-b text-foreground"
          }`}
        >
          {navItems}
        </div>
